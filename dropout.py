...
class DropBlock2CSDN(_DropoutNd):
...
class DropBlockold(_DropoutNd):
    __constants__ = ["p", "block_size", "inplace"]  # 新增block_size
    block_size: int  # 块大小

    def __init__(self, p: float = 0.1, block_size: int = 7, inplace: bool = False) -> None:
        super().__init__(p=p, inplace=inplace)  # 复用p作为丢弃概率γ
        self.block_size = block_size

    def forward(self, input: Tensor) -> Tensor:
        if not self.training:  # 测试阶段也需应用（MC-DropBlock要求）
            return self._apply_dropblock(input)
        else:  # 训练阶段应用
            return self._apply_dropblock(input)

    def _apply_dropblock(self, x: Tensor) -> Tensor:
        batch_size, channels, height, width = x.shape
        # 1. 从伯努利分布采样掩码（γ = self.p）
        mask = torch.bernoulli(torch.full((batch_size, channels, height, width), 1 - self.p, device=x.device))
        # 2. 对掩码中为0的位置，生成block_size×block_size的连续块并置零
        block = torch.zeros((self.block_size, self.block_size), device=x.device)
        mask = F.conv2d(mask, block.unsqueeze(0).unsqueeze(0), padding=self.block_size // 2, groups=channels) > 0
        mask = mask.float()
        # 3. 归一化：count(M)/count_ones(M)
        count_ones = mask.sum(dim=(2, 3), keepdim=True)
        count_total = height * width
        mask = mask * (count_total / count_ones.clamp(min=1e-8))  # 避免除零
        # 应用掩码
        return x * mask if not self.inplace else x.mul_(mask)
